    ...
    if len(data.ctrl) > 0:
        for i in range(action.shape[0]):
            ...
            if model.actuator_biastype[i] == 0:
                ...
                data.ctrl[i] = action[i]#data.qpos[idx] + action[i]
    ...
    if model.nmocap > 0 and model.eq_data is not None:
        for i in range(model.eq_data.shape[0]):
            if model.eq_type[i] == mujoco.mjtEq.mjEQ_WELD:
                #model.eq_data[i, :7] = np.array([0.00000000e+00, 0.00000000e+00, 0.00000000e+00, 1.11022302e-16, 0.00000000e+00, 1.11022302e-16, 1.00000000e+00])
                #model.eq_data[i, :7] = np.array([0.0, 0.0, 0.0, 0.2982001, 0.1311989, 0.44540031, 0.0]) #-->welded with base_mount ur3e
                #model.eq_data[i, :7] = np.array([0.0, 0.0, 0.0, 7.00000000e-04, -1.35720000e-01, 0.0, 0.0]) #-->welded with base_mount ur5e
...
def get_body_jacp(model, data, body_id):
    """Return the Jacobian' translational component of the end-effector of
    the corresponding body id.
    """
    jacp = np.zeros((3, model.nv))
...
def get_body_jacr(model, data, body_id):
    """Return the Jacobian' rotational component of the end-effector of
    the corresponding body id.
    """
    jacr = np.zeros((3, model.nv))
...
def get_body_xquat(model, data, name):
    body_id = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_BODY, name)
    return data.xquat[body_id]

def get_body_xvelp(model, data, name):
    body_id = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_BODY, name)
    jacp = get_body_jacp(model, data, body_id)
    xvelp = jacp @ data.qvel
    return xvelp
...
def get_body_xvelr(model, data, name):
    body_id = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_BODY, name)
    jacp = get_body_jacr(model, data, body_id)
    xvelr = jacp @ data.qvel
    return xvelr
...
def get_geom_contacts(model: MjModel, data: MjData, geom1: str, geom2: str):
    geom1_id = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_GEOM, geom1)
    geom2_id = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_GEOM, geom2)
    for i in range(data.contact.dist.shape[0]):
        if data.contact.geom1[i] == geom1_id and data.contact.geom2[i] == geom2_id:
            return True
    else:
        return False
                
def get_contact_dist(model: MjModel, data: MjData, geom1: str, geom2: str):
    geom1_id = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_GEOM, geom1)
    geom2_id = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_GEOM, geom2)
    indices = np.array([])
    for i in range(data.contact.dist.shape[0]):
    ...
    if indices.shape[0]>=1:
        dist = np.sum( data.contact.dist[indices] )/indices.shape[0]
        return dist
    else:
        return 0

def get_contact_pos(model: MjModel, data: MjData, geom1: str, geom2: str):
    geom1_id = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_GEOM, geom1)
    geom2_id = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_GEOM, geom2)
    indices = np.array([])
    for i in range(data.contact.dist.shape[0]):
    ...
    indices = indices.astype(int)
    if indices.shape[0]>=1:
        pos = np.sum( data.contact.pos[indices,:]/indices.shape[0], axis=0 )
        return pos
    else:
        return np.zeros(3)
